<x-app-layout>
    <div class="max-w-7xl mx-auto p-4 sm:p-6 lg:p-8">
        @include('components.page-header', [
            'title' => 'Kelola Dokumen Capstone',
        ])

        <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
            <div class="p-6 bg-white border-b border-gray-200">
                <form action="{{ route('member.capstones.update', $capstone->id) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                ...
                                <div class="relative">
                                    <input type="text" name="kode_kelompok" id="kode_kelompok" 
                                        ...
                                        value="{{ old('kode_kelompok', $capstone->kode_kelompok) }}" 
                            ...
                            <div>
                                ...
                                <select name="kategori" id="kategori" 
                                    ...
                                    @foreach(['SI', 'Jaringan', 'Embedded', 'Game'] as $kat)
                                        <option value="{{ $kat }}" {{ old('kategori', $capstone->kategori) == $kat ? 'selected' : '' }}>
                                            {{ $kat }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div>
                                ...
                                <div class="relative">
                                    <input type="text" name="anggota2_nim" id="anggota2_nim" 
                                        ...
                                        value="{{ old('anggota2_nim', $capstone->anggota2_nim) }}">
                            ...
                            <div>
                                ...
                                <div class="relative">
                                    <input type="text" name="anggota3_nim" id="anggota3_nim" 
                                        ...
                                        value="{{ old('anggota3_nim', $capstone->anggota3_nim) }}">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div>
                                <label class="block text-gray-700 text-sm font-bold mb-2" for="{{ $doc }}">
                                    Dokumen {{ strtoupper($doc) }}
                                </label>
                                
                                @if($capstone->{$doc.'_path'})
                                <div class="mb-2">
                                    <a href="{{ Storage::url($capstone->{$doc.'_path'}) }}" 
                                ...
                                <p class="text-xs text-gray-500 mt-1">Upload file baru untuk mengganti dokumen yang ada</p>
                                @error($doc)
                                    <p class="text-red-500 text-xs italic">{{ $message }}</p>
                                @enderror
                            </div>
    ...
    <script>
        ...
        const currentKode = '{{ $capstone->kode_kelompok }}';
            ...
            checkTimeout = setTimeout(() => {
                fetch(`/api/check-nim/${nim}`)
                    ...
                    .then(data => {
                        ...
                        // For edit, we need to check if the NIM is the same as current member
                        const isCurrentMember = nim === '{{ $capstone->anggota2_nim }}' || 
                                             nim === '{{ $capstone->anggota3_nim }}';
        
                        if (data.exists && (!data.inTeam || isCurrentMember)) {
                            statusEl.querySelector('.success-icon').classList.remove('hidden');
                            messageEl.textContent = 'NIM valid';
                            messageEl.classList.remove('text-red-500');
                            messageEl.classList.add('text-green-500');
                        } else if (data.exists && data.inTeam && !isCurrentMember) {
        ...
        function validateKodeKelompok(input) {
            ...
            // If code hasn't changed, don't check for uniqueness
            if (kode === currentKode) {
                statusEl.classList.remove('hidden');
                statusEl.querySelector('.success-icon').classList.remove('hidden');
                messageEl.textContent = 'Kode kelompok valid';
                messageEl.classList.remove('hidden');
                messageEl.classList.add('text-green-500');
                return;
            }
