        ...
        <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
            @foreach($loans as $loan)
                <div class="bg-white rounded-lg shadow-lg overflow-hidden border border-gray-200 hover:shadow-xl transition-shadow duration-300">
                    <!-- Lost Book Status Banner -->
                    @php
                        $lostBook = \App\Models\LostBook::where('book_loan_id', $loan->id)
                            ->where('replacement_status', 'awaiting_verif')
                            ->first();
                    @endphp
                    
                    @if($lostBook)
                        <div class="bg-purple-500 text-white px-4 py-2 text-center font-semibold">
                            ...
                            @if($lostBook)
                                <div class="mt-2 p-3 bg-purple-50 rounded-md">
                                    <p class="text-sm text-purple-700">
                                        <span class="font-semibold">Status:</span> Menunggu verifikasi penggantian buku
                                    </p>
                                    <p class="text-sm text-purple-700 mt-1">
                                        <span class="font-semibold">Tanggal Lapor:</span> {{ \Carbon\Carbon::parse($lostBook->date_reported)->format('d M Y') }}
                                    </p>
                                </div>
                            @endif
                        </div>
